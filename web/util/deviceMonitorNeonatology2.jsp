	...
	private String printMonitor(String personid,String deviceid,java.sql.Timestamp ts,double baseFontSize,double cellHeight){
		...
		try{
			...
			while(rs.next() && results.size()<nSamplesToAverage){
				...
				else if(code.equalsIgnoreCase("9279-1") && (fValue>69)){
					continue;
				}
				else if(code.equalsIgnoreCase("73798-1") && (fValue>20)){
					continue;
				}
				else if(code.equalsIgnoreCase("59408-5") && (fValue<30 && fValue>100)){
					continue;
				}
			...
			if(lastMovement!=null && new java.util.Date().getTime()-lastMovement.getTime()<SH.getTimeSecond()*30){
				s+="<tr height='"+cellHeight*1.2/12+"px'>"+sMovementTitle+(SH.c(personid).length()==0?"":AdminPerson.getFullName(personid))+"</td></tr>";
				s+="<tr height='"+cellHeight*0.8/12+"px'>"+sSmallMovementTitle+"["+SH.cs("deviceAlias."+deviceid,deviceid)+"] "+deviceid+"</td></tr>";
			}
			else{
				s+="<tr height='"+cellHeight*1.2/12+"px'>"+sTitle+(SH.c(personid).length()==0?"":AdminPerson.getFullName(personid))+"</td></tr>";
				s+="<tr height='"+cellHeight*0.8/12+"px'>"+sSmallTitle+"["+SH.cs("deviceAlias."+deviceid,deviceid)+"] "+deviceid+" ID: "+personid+"</td></tr>";
			}
