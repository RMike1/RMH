	             	...
	             	<%
	             		if(MedwanQuery.getInstance().getConfigInt("enableRwanda",1)==1){
		             		%>
		                       ...
		                       <option value="rssbrmhcsv" <%=defaultmodel.equalsIgnoreCase("rssbrmhcsv")?"selected":""%>><%=getTranNoLink("web","rssbrmhcsvmodel",sWebLanguage)%></option>
		              		<%
	             		}
...
function doPrintPdf(invoiceUid) {
    ...
    else if(EditForm.PrintModel.value=='rssbrmhcsv'){
        var url = "<c:url value='/util/csvDocs.jsp'/>?invoiceuid=" + invoiceUid + "&ts=<%=getTs()%>&docid=invoice.rssbrmh";
    	location.href=url;
    }
